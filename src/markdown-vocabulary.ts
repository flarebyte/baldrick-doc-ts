...
import { ModuleInfo, ProjectNgram, ProjectVocabulary } from './parser-model.js';

export const toVocabularyTable = (
  projectVocabulary: ProjectVocabulary
): string => {
  const rows: [string, string, string][] = projectVocabulary.vocabulary.map(
...
export const toVocabularyRankTable = (projectNgram: ProjectNgram): string => {
  const rows: [string, string][] = projectNgram.ngram.map((v) => [
